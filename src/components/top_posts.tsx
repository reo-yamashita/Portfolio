import { TypeTopPosts } from '@@/allTypes/gatsby-type'
import React from 'react'

const TopPosts: React.FC<TypeTopPosts> = ({title, color, children}) => {

  return(
    <div className="select-none top_posts_basis p-3 ">
      <div className={`border-blue-900 border rounded-lg px-8 pt-4 pb-6 hover:bg-${color}
      hover:shadow-md border-opacity-25 hover:border-opacity-0 transition_ease_in`}>
        <h2 className="text-2xl tracking-wider font-medium text-indigo-900">{title}</h2>
        <p className="mt-3 ml-3 text-base leading-none tracking-wide">{children}</p>
      </div>
    </div>
  )
}

export default TopPosts